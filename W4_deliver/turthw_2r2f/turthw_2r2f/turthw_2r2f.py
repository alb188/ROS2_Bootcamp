...
class TurtlePreGoal(Node):
    ...
    def pose_callback(self, msg):
        ...
        if diff_of_xpositions < 1.0 and diff_of_ypositions < 1.0:
            twist = Twist()
            twist.linear.x = 0.2  # Stop the robot
            twist.angular.z = 0.0  # Stop rotation
            self.publisher.publish(twist)

        elif diff_of_xpositions >= 1.0 and diff_of_orientations < 0.707:
            twist = Twist()
            twist.linear.x = 0.0  # Stop the robot
            twist.angular.z = -0.2  # Stop rotation
            self.publisher.publish(twist)

        elif diff_of_ypositions < 1.0 and diff_of_orientations >= 0.707:
            twist = Twist()
            twist.linear.x = 0.2  # Stop the robot
            twist.angular.z = 0.0  # Stop rotation
            self.publisher.publish(twist)
