...
class TurtleSpiral(Node):
	...
	def pose_callback(self, msg):
		...
		if diff_of_xpositions < 8.0 and diff_of_ypositions < 8.0 and (diff_of_xpositions > 1.0 or diff_of_ypositions > 1.0):
			...
			#msgt.angular.z = 0.0
			#msgt.linear.x = 0.0
			#self.get_logger().info(f"diff_x={diff_of_xpositions}, diff_y={diff_of_ypositions}. Pose out of bounds, shutting down...")
			#self.get_logger().info(f"initx={self.initial_position.x}, currx={current_position.x}, inity={self.initial_position.y}, curry={current_position.y}")
			#self.destroy_node()
			#rclpy.shutdown()
